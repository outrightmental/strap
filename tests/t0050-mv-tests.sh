...
test_expect_success 'Basic move command' '
	...
	[[ -e $STRAP_DIR/cred2.yml && ! -e $STRAP_DIR/cred1.yml ]]
'

test_expect_success 'Directory creation' '
	"$STRAP" mv cred2 directory/ &&
	[[ -d $STRAP_DIR/directory && -e $STRAP_DIR/directory/cred2.yml ]]
'
...
test_expect_success 'Directory creation with file rename and empty directory removal' '
	...
	[[ -d $STRAP_DIR/"new directory with spaces" && -e $STRAP_DIR/"new directory with spaces"/cred.yml && ! -e $STRAP_DIR/directory ]]
'

test_expect_success 'Directory rename' '
	"$STRAP" mv "new directory with spaces" anotherdirectory &&
	[[ -d $STRAP_DIR/anotherdirectory && -e $STRAP_DIR/anotherdirectory/cred.yml && ! -e $STRAP_DIR/"new directory with spaces" ]]
'

test_expect_success 'Directory move into new directory' '
	"$STRAP" mv anotherdirectory "new directory with spaces"/ &&
	[[ -d $STRAP_DIR/"new directory with spaces"/anotherdirectory && -e $STRAP_DIR/"new directory with spaces"/anotherdirectory/cred.yml && ! -e $STRAP_DIR/anotherdirectory ]]
'
...
test_expect_success 'Multi-directory creation and multi-directory empty removal' '
	...
	[[ ! -d $STRAP_DIR/"new directory with spaces"/anotherdirectory && ! -d $STRAP_DIR/new1/new2/new3/new4 && -e $STRAP_DIR/cred.yml ]]
'
