...
# test.
#
# If some cleanup command fails, the test will not strap. With --immediate, no
...
test_done() {
	...
	0)
		...
		if test $test_remaining -gt 0; then
			say_color strap "# passed all $msg"
		fi
...
# Prepare test area.
test_dir="trash/$(basename "$SHARNESS_TEST_FILE" ".$SHARNESS_TEST_EXTENSION")"
